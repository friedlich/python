...
def parse_type(content):
    return re.findall(r"docType.*?\:.*?\'(.*?)\'\,", content)[0]  # r可以忽略用'\'代替'\\'，u可以把引号里面的字符转成unicode编码，这两步都需要的时候就在引号前面输入ur，而ru就报错
...
def main():
    ...
    content = fetch_url(url)
    with open(r'c:/Users/asus/Desktop/Python/风变Python爬虫精进/爬虫阶段练习/19年7月/7.08/baiduwenku2', 'w', encoding='utf8') as f:
        f.write(content)
    # print(content)
